var container = document.getElementById("female-cancer-rates");
...
var femalCancerMapSvg = d3.select("#female-cancer-rates").append("svg")
    .attr("width", w)
    .attr("height", h);

// RGB range colour scheme from color brewer
// note we haven't applied the domain yet as we haven't got the data
var femaleCancerRateColourDomain = d3.scaleLinear()
                                    .range(['rgb(247,247,247)','rgb(204,204,204)','rgb(150,150,150)','rgb(99,99,99)','rgb(37,37,37)']);

d3.csv("https://poc275.me/tufte-d3js/data/Female Mortality Rate All Cancers.csv").then(function(data) {
    ...
    femaleCancerRateColourDomain.domain(d3.ticks(min, max, 7));
    ...
    d3.json("https://poc275.me/tufte-d3js/data/gz_2010_us_050_00_5m.json").then(function(json) {
        ...
        femalCancerMapSvg.selectAll("path")
            .data(json.features)
            .enter()
            .append("path")
            .attr("d", path)
            ...
            .style("fill", function(d) {
                ...
                if(value) {
                    return femaleCancerRateColourDomain(value);
                } else {
                    return "#fff";
                }
            })
        ...
        var legendData = [0, 57.5, 148.6, 167.3, 304];

        // add color legend
        femalCancerMapSvg.selectAll("rect")		                        	
            .data(legendData)
            .enter()
            .append("rect")
            ...
            .attr("stroke", "#aaa")
            .attr("stroke-width", "0.25")
            .attr("fill", function(d) { return femaleCancerRateColourDomain(d); });

        femalCancerMapSvg.selectAll("text")
